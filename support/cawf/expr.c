...
char *
Asmcode(s, c)
	char **s;			/* pointer to character after '\\' */
	char *c;			/* code destination (c[3]) */
{
	char *s1;
	...
	int nx;				/* number index */
{
	...
	if (nx >= Nnr) {
		(void) sprintf(buf, " bad Delnum(%d) index", nx);
					 ...
					 * 1 = enter, don't find */
{
	...
	while (low <= hi) {
		mid = (low + hi) / 2;
		if ((cmp = strncmp(c, Numb[mid].nm, 2)) < 0)
			hi = mid - 1;
		else if (cmp > 0)
			low = mid + 1;
		else {
	...
	if ( ! e)
		return(-1);
	if (Nnr >= MAXNR)
		Error(FATAL, LINE, " out of number registers at ", c);
	if (Nnr) {
		if (cmp > 0)
			mid++;
...
Findparms(n)
	char *n;                        /* parameter name */
{
	char c[3];			/* character buffer */
