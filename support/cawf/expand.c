...
Expand(line)
	char *line;
{
	char buf[2*MAXLINE];		/* line buffer */
	char cmd[4];                    /* nroff command */
	...
	(void) sprintf(buf, ".^= %d %s", NR, Inname);
	Pass2(buf);
	...
	for (lp = line; *lp; ) {
	    ...
	    /*
	     * Argument count comparison: - "^\.i[ef] !?\\n\(\.\$[<=>][0-9] "
	     */
			iflen = strlen(".if \\n(.$=n ") + invert;
			n1 = (int)(*(lp + iflen - 2) - '0');
			switch (*(lp + iflen - 3)) {
	     ...
	     */
			...
			if (n1 >= 0 && n1 < nargs)
				...
				s1 = "";
			if ((s2 = strchr(lp + iflen, '\'')) != NULL) {
				n2 = s2 - lp - iflen;
				if (strncmp(s1, lp + iflen, n2) == 0)
					cond = 1;
				iflen += n2 + 1;
			}
		}
	     ...
	     */
			cond = 1;
			iflen = 0;
			(void) sprintf(buf, ".tm unknown .if/.ie form: %s", lp);
			lp = buf;
		}
	   /*
	    ...
	    */
		...
		if (cond == 0)
			...
			(void) sprintf(buf, " macro nesting > %d", MAXSP);
