...
// - made filenames lower case, DOS/Windows doesn't care, but UNIX does
// - use endian aware functions from the lib to write binary data into files
// - fixed the usual / \ path problem
//
// UM 1999-06-30
// - added string defines for path/extension to sysdep.h, use those to
//   build filenames
...
void SupplyExt(char *str, char *ext)
{
	...
	p = strrchr(str, PATHSEPERATOR);	// last path seperator
	q = strrchr(str, EXTSEPERATOR);		// last extension seperator
	if (p && q && q < p)	// if is a backslash, and the . is before it
        {
		strcat(str, EXTSEPERATOR_STR);
...
int main(int argc, char **argv)
{
	...
	if (st)
	{
		...
		while (st && (p = fgets(buffer, 256, st)))
		{
			...
			else if (!stricmp("FLATS", secname) ||
				 !stricmp("TEXTURES", secname))
			{
				tok = strtok(p, " \t,");
				if (!tok)
				{
